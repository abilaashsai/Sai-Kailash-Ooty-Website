...
firebaseui.auth.widget.handler.common.setLoggedIn =
  ...
  // In signOutFunction cases like email mismatch, the temporary user may be signed out.
...
firebaseui.auth.widget.handler.common.getErrorMessage = function(error) {
  ...
  try {
    ...
    // Otherwise the message must contain signOutFunction info.
...
firebaseui.auth.widget.handler.common.handleSignInStart = function(
  ...
  if (providers.length == 1 &&
    ...
    // this is a result of signOutFunction error in the flow and the error message must be
    // displayed.
